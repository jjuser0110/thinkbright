                ...
                <tbody>
                  @foreach ($categories as $category)
                    <tr>
                      <td>{{ $category->name ?? "" }}</td>
                      <td>
                        <a style="text-decoration: none; color: inherit;" href="{{ route('category.edit', $category) }}" title="Edit">
                          <i class="fas fa-pencil-alt"></i>
                        </a>
                        &nbsp;&nbsp;
                        @if(Auth::user()->role == "admin")
                          <button type="button" style="background: none; padding: 0px; border: none; color: inherit;" onclick="if(confirm('Are you sure you want to delete?')){ window.location.href='{{ route('category.destroy', $category) }}' }"><i class="fas fa-trash-alt"></i></button>
                        @endif
                      </td>
                    </tr>
                    @endforeach
